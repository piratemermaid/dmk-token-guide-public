...
const updateEventDates = require("./updatedb/eventDates");
const updateEventReqLevels = require("./updatedb/eventReqLevels");
...
async function updateEvent() {
    ...
    if (EVENT.CHARACTERS.length > 0) {
        await addEventCharacters();
    }

    if (EVENT.SPECIAL_TOKENS.length > 0) {
        await addEventSpecialTokens();
    }

    await updateEventReqLevels();
...
async function removeEvent() {
    await knex(TABLES.EVENT).where({ id: 1 }).update({ name: "", type: "" });
    await knex(TABLES.EVENT_GROUPS).where({ event_id: 1 }).del();
    await knex(TABLES.EVENT_CHARACTERS).where({ event_id: 1 }).del();
    await knex(TABLES.EVENT_SPECIAL_TOKENS).where({ event_id: 1 }).del();
}

async function addNewEvent() {
    const { NAME, TYPE } = EVENT;
    await knex(TABLES.EVENT).where({ id: 1 }).update({
        name: NAME,
        type: TYPE
    });
